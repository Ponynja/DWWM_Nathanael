...
// on déclare le widget
    ...
    { //méthode déclenchée à l'activation du plug-in
        ...
        $wpdb->query("CREATE TABLE IF NOT EXISTS {$wpdb->prefix}helloworld_nom (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255) NOT NULL);");
    }
    ...
    { //méthode déclenchée à la suppression du module
        ...
        $wpdb->query("DROP TABLE IF EXISTS {$wpdb->prefix}helloworld_nom;");
    }

    public function save_comm()
    {
        // var_dump($_POST);
        if (isset($_POST['helloworld_comm']) && !empty($_POST['helloworld_comm']) && isset($_POST['helloworld_name']) && !empty($_POST['helloworld_name'])) {
            global $wpdb;
            $name = $_POST['helloworld_name'];
            $comm = $_POST['helloworld_comm'];
            $rowcomm = $wpdb->get_row("SELECT * FROM {$wpdb->prefix}helloworld_commentaire WHERE comm = '$comm'");
            $rowname = $wpdb->get_row("SELECT * FROM {$wpdb->prefix}helloworld_nom WHERE name = '$name'");
            ...
            if (is_null($rowname)) {
                $wpdb->insert("{$wpdb->prefix}helloworld_nom", array('name' => $name));
            }
