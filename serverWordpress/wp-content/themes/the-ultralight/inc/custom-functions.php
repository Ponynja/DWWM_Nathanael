...
function the_ultralight_scripts() {
    ...
    $the_ultralight_font_query_args = array('family' => 'Roboto:300,300i,400,400i,500,500i,700,700i');
    wp_enqueue_style('the-ultralight-google-fonts', add_query_arg($the_ultralight_font_query_args, "//fonts.googleapis.com/css"));
...
function the_ultralight_change_comment_form_default_fields( $fields ){    
    ...
    $fields['author'] = '<p class="comment-form-author"><label class="screen-reader-text" for="author">' . esc_html__( 'Name*', 'the-ultralight' ) . '</label><input id="author" name="author" placeholder="' . esc_attr__( 'Name*', 'the-ultralight' ) . '" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>';
    
    $fields['email'] = '<p class="comment-form-email"><label class="screen-reader-text" for="email">' . esc_html__( 'Email*', 'the-ultralight' ) . '</label><input id="email" name="email" placeholder="' . esc_attr__( 'Email*', 'the-ultralight' ) . '" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>';
...
function the_ultralight_change_comment_form_defaults( $defaults ){    
    $defaults['comment_field'] = '<p class="comment-form-comment"><label class="screen-reader-text" for="comment">' . esc_html__( 'Comment', 'the-ultralight' ) . '</label><textarea id="comment" name="comment" placeholder="' . esc_attr__( 'Comment', 'the-ultralight' ) . '" cols="45" rows="8" aria-required="true"></textarea></p>';
        ...
        <div class="welcome-message notice notice-info">
            <div class="notice-wrapper">
                <div class="notice-text">
                    ...
                    <p><?php printf( __( '%1$s is now installed and ready to use. Click below to see theme documentation, plugins to install and other details to get started.', 'the-ultralight' ), $name ) ; ?></p>
...
if( ! function_exists( 'the_ultralight_restore_admin_notice' ) ) :
/**
 * Restoring admin notice on theme switch
 */
function the_ultralight_restore_admin_notice(){
    update_option( 'the_ultralight_admin_notice', false );
}
endif;
add_action( 'after_switch_theme', 'the_ultralight_restore_admin_notice' );